...
import com.zyq.mall.dao.ProductMapper;
import com.zyq.mall.enums.ProductStatusEnum;
import com.zyq.mall.enums.ResponseEnum;
import com.zyq.mall.form.CartAddForm;
import com.zyq.mall.form.CartUpdateForm;
import com.zyq.mall.pojo.Cart;
import com.zyq.mall.pojo.Product;
import com.zyq.mall.service.ICartService;
import com.zyq.mall.vo.CartProductVo;
...
public class CartServiceImpl implements ICartService {
	...
	public ResponseVo<CartVo> add(Integer uid, CartAddForm form) {
		...
		if (StringUtils.isEmpty(value)) {
			...
			//已经有了，数量+1
			cart = gson.fromJson(value, Cart.class);//反序列化（json，需要转成的对pojo）
			cart.setQuantity(cart.getQuantity() + quantity);
		}

		opsForHash.put(redisKey,
				String.valueOf(product.getId()),
				gson.toJson(cart));//将对象序列化专程json字符成
	...
	public ResponseVo<Integer> sum(Integer uid) {
		Integer sum = listForCart(uid).stream()//便利购物车
				.map(Cart::getQuantity)//取出数量
